...
class ConstrutorFase(metaclass=Singleton):
    ...
    def constroiFase(self, nivel: str, dificuldade: int) -> Fase:
        ...
        for i in range(num_inimigos_p):
            inimigos_pessoa.append(InimigoPessoa(lista_spawn_p.pop(randrange(len(lista_spawn_p))), sprites=self.__biblioteca.getSprites(nivel)[
                                   'inimigo_pessoa']))  # adiciona um inimigo de um spawn aleatorio
        for i in range(num_inimigos_o):
            inimigos_obstaculo.append(InimigoObstaculo(lista_caminhos_o.pop(randrange(len(
                lista_caminhos_o))), sprites=self.__biblioteca.getSprites(nivel)['inimigo_obstaculo']))
